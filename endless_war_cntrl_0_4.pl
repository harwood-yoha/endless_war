...
if( WITH_SERVER){
	...
	$ngm->init_client;
		print " \n SERVER PORT = ".$ngm->{client}->{port};


}
if( WITH_MYSQL_SERVER){
	my $ss = MYSQL_SERVER_SCRIPT;
	my $s = `ps aux | grep $ss` ;
	die "\n ERR: SERVER NOT RUNNING '".MYSQL_SERVER_SCRIPT."'\n" unless  $s =~ 'perl '.MYSQL_SERVER_SCRIPT;
	$mysql_serv = WAR::Client->new; 
	$mysql_serv->init('mysql');
	print " \n MYSQL PORT = ".$mysql_serv->{mysql_port};
...
while ($#doc_sorted > 2) {
	...
	if( WITH_SERVER){
		...
		#	$ngm->{x_mysql}->send_mesg("select * from war_diary where war_diary_id < $src_doc;");
			...
			$upper_rnge = MYSQL_LINES if $upper_rnge < MYSQL_LINES;
			my %opt;
			($opt{to},$opt{from}) = ($upper_rnge,$lower_rnge);	
			$mysql_serv->send_mysql_command(\%opt);
		}	
		usleep(800000);
